...
### 使用try catch 捕获异常流程(todo) 


### 函数选择器(selector) 
调用智能合约本质上是向目标合约发送了一段calldata，在remix中发送一次交易后，可以在详细信息中看见input即为此次交易的calldata
...
运行以下函数，返回值为0x6a627842，该值为mint函数选择器
```solidity
 function mintSelector() public pure returns(bytes4 mSelector){
        return bytes4(keccak256("mint(address)"));
    }
```
函数选择器计算过程为，对函数签名进行keccak256 hash运算，hash运算结果取前四个字节作为函数选择器

智能合约调用calldata为 函数选择器+函数参数
